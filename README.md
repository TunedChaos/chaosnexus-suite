...
On Codeberg it is published as [chaosnexus-suite](https://codeberg.org/TunedChaos/chaosnexus-suite) ([GitHub mirror](https://github.com/TunedChaos/chaosnexus-suite)).

> **Status:** early public alpha. Create the empty Codeberg repo when ready; subtree sync and Release publish are wired in release CI.

## Downloads

Canonical installers are attached to **Codeberg Releases** on this repository (tag `suite-vX.Y.Z`):
...
GitHub Releases mirror the same assets for discoverability. Models (GGUF) are **not** baked in - download on first Models use (Hugging Face / Tuned).
...
AGPL-3.0-or-later. Contribute on Codeberg.
